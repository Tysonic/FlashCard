...
const StartQuiz = ({ route, navigation }) => {
    ...
    useEffect(() => {
        AsyncStorage.getItem(ANSWER_KEY)
            .then(s => {
                ...
                if (data !== null) {
                    const returnVal = Object.keys(data).filter(s => s.split(" ")[0] === deck.title).map(s=>data[s])
    ...
    const handleAnswer = (text) => {
        AnswerQustion({ deck: deck.title, question: selectedQuestion.question, passed: text })
        
        const data = fromDb
        if(data){data.push(text)
        setFromDb(data)}
                ...
                displayResult
                    ...
                    <View style={{ flex: 1 }}>
                        ...
                        <View style={Styles.VerticalAlignCenter}>
                            ...
                            <TouchableOpacity onPress={() => setDisplayQuestion(!displayQuestion)}>
                                {displayQuestion ?
                        ...
                        <View style={{ marginBottom: 50 }}>
                            ...
                            <TouchableOpacity onPress={() => handleAnswer(false)}>
                                <Text style={[Styles.button, { color: 'red', borderColor: 'red' }]}>
                                    Incorrect
                                </Text>
                            </TouchableOpacity>
                        </View>
